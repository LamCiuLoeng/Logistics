...
class Customer(DeclarativeBase, SysMixin, CRUDMixin):
    ...
    display_name = Column(Text, doc = u'显示名称')
    province_id = Column(Integer, ForeignKey('master_province.id'), doc = u'地址省份')
    province = relation(Province)
    city_id = Column(Integer, ForeignKey('master_city.id'), doc = u'地址市')
    city = relation(City)

    address = Column(Text, doc = u'详细地址')
